...
- **电子书阅读器**（EPUB/MOBI/AZW/FB2/CBZ）—— 基于 foliate-js 统一渲染引擎：**排版设置面板（Aa）**含主题（明亮/羊皮纸/夜间）、字体族/字重/行距/字间距/边距/对齐/亮度共 8 项、字号缩放、点击翻页/键盘翻页、全文 FTS5 搜索、阅读位置记忆、有声书跟读高亮
- **电子书标注** —— 内文选中文字即可高亮（4 色可选）、写笔记，位置以 **EPUB CFI** 存储（与渲染引擎无关）；标注面板支持跳转原文 / 改色 / 编辑笔记 / 删除；**书摘可一键发送到任意会话**，文本带书名出处
- **阅读器查词 / 翻译** —— 选中文字即可让 AI 翻译或解释词义，结果流式显示在右侧面板，可复制或直接发送到会话；复用「设置 → AI 管理」里已配置的接口
- **有声书（EPUB → 语音）** —— TTS 逐句朗读 + 当前句高亮 + 自动翻页，跨章节连续播放；点击朗读时可选起始位置（继续上次进度 / 当前阅读位置 / 从头开始）
- **有声书导出** —— 将当前章节所有句子批量合成 → FFmpeg 拼接 → 导出 WAV/MP3 音频文件
- **收听历史** —— 记录每次聆听的时长/句数/语音，统计总时长与会话数（仅统计真正播放的时间，不含暂停）
...
**v0.4.0**（2026-09-13）—— 阅读器排版设置 + 有声书高亮内核重构 + 电子书标注 + 查词/翻译 + 朗读扩展到 TXT/MD/PDF

- **阅读器排版设置**：新增 Aa 设置面板，含主题（明亮/羊皮纸/夜间）、字体族、字重、行距、字间距、边距、对齐、亮度共 8 项；原独立行距下拉并入面板
- **有声书增强**：点击朗读时弹出起始位置三选一（继续上次进度/当前阅读位置/从头开始）；收听历史面板上线（此前数据层就绪但无入口）；收听时长不再把暂停时间计入
- **高亮内核重构**：改用 Range + CSS Custom Highlight API，不再用 `surroundContents` 改写 EPUB 正文 DOM；朗读与高亮共用统一正文源，消除「某些句子漏高亮」与「嵌套块导致同一句重复朗读」两个既有缺陷；并修复「停止朗读后底纹残留」（根因是 foliate 的 `scrollToAnchor` 会按调用方要求把当前句整段选中，画出与高亮极像的选区底纹）
- **电子书标注**：选中文字即可高亮（4 色）与写笔记，位置以 EPUB CFI 存储；新增标注管理面板（跳转原文/改色/编辑笔记/删除）；**书摘一键发送到会话**（阅读器在独立预览窗口中运行，发送经主进程中继给主窗口执行）
- **阅读器查词 / 翻译**：选中文字 → AI 翻译或解释词义，结果流式显示在右侧面板，可复制或发送到会话；AI 配置直接复用「设置 → AI 管理」
- **朗读支持扩展到 TXT / Markdown / PDF**：此前只有 EPUB 能朗读。现将朗读编排抽为与渲染引擎无关的共享层（`useAudiobookSession` + `AudiobookOverlay`），三种格式一并接入 —— TXT 与 Markdown 用纯 DOM 阅读器，PDF 以「一页 = 一个 section」复用现成的翻章逻辑，**可自动跨页连读**
- **修复 4 项朗读缺陷**：`startReading` 未裁剪越界恢复索引导致静默跳读；TXT 以空行开头时**整本被判不可读**（点朗读毫无反应）；PDF 并发翻页等待时 Promise 永久挂起；点「上一章/下一章」后停播
- **渲染层单元测试**：引入 vitest + jsdom（此前渲染层零测试），覆盖页码编码、滚动位置换算、阅读位置持久化与正文提取，共 54 条
- **修复主窗口引用失效**：handler 注册早于建窗导致各处拿到永久 `null` 快照，标题栏最小化/最大化/关闭三个按钮与两处 IPC 推送因此长期失效（macOS 上按钮不渲染故未被发现）
